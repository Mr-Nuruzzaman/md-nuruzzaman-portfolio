...
**SEO / meta.** `src/lib/metadata.ts` builds `siteMetadata` and `personJsonLd` from `profile` + `SITE_URL`. `SITE_URL` comes from `NEXT_PUBLIC_SITE_URL` env (fallback `https://md-nuruzzaman-portfolio.vercel.app`) and feeds metadata, `sitemap.ts`, `robots.ts`, and JSON-LD. OG image is generated by `src/app/opengraph-image.tsx` (next/og); `apple-icon.tsx`, `not-found.tsx`, and `loading.tsx` also exist.
